...
import { AdminValuesApi } from "../API/values"
...
const ICON_OPTIONS: Array<{ key: IconKey; label: string }> = [
  { key: "Shield", label: "Shield — Trust / Safety" },
  { key: "Award", label: "Award — Excellence" },
  { key: "CheckCircle", label: "CheckCircle — Principles" },
  ...
  { key: "Globe", label: "Globe — Inclusion" },
...
export default function AdminValuesPage() {
  ...
  const load = async () => {
    try {
      setLoading(true)
      setError(null)
      const data = await AdminValuesApi.list()
  ...
  const startEdit = (row: ValueItem) => {
    setEditingId(row.id)
    setTitle(row.title)
    setDescription(row.description)
    setIcon(row.icon as IconKey)
    setColor(row.color)
    setBgColor(row.bg_color)
  ...
  const upsert = async () => {
    ...
    try {
      ...
      if (editingId) {
        const updated = await AdminValuesApi.update(editingId, payload)
        setRows((prev) => prev.map((row) => (row.id === updated.id ? updated : row)))
      } else {
        const created = await AdminValuesApi.create(payload)
  ...
  const remove = async (id: number) => {
    ...
    try {
      await AdminValuesApi.remove(id)
    ...
    <section className="space-y-8">
      <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900 via-slate-900/80 to-slate-950 p-6 shadow-2xl">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-semibold">Core Values</h1>
            <p className="mt-2 max-w-2xl text-sm text-slate-300">
              Manage the “Our Core Values” cards on the About page.
            </p>
          </div>
      ...
      <Card className="shadow-xl">
        <CardHeader>
          <CardTitle className="text-2xl">
            {editingId ? "Edit Value" : "Add a Core Value"}
          </CardTitle>
          <p className="mt-1 text-sm text-slate-500">
            Icons map directly to the front-end cards.
          </p>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          <label className="space-y-2">
            ...
            <textarea
              ...
              rows={3}
              className="w-full rounded-md border border-slate-200 px-3 py-2"
              placeholder="Value description shown on the card."
            />
          </label>
          ...
          <div className="flex flex-wrap gap-2">
            <Button onClick={upsert} disabled={saving}>
              {saving ? "Saving..." : editingId ? "Update Value" : "Add Value"}
            </Button>
            {editingId ? (
      ...
      <Card className="shadow-xl">
        <CardHeader>
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <CardTitle className="text-2xl">Current Values</CardTitle>
            <input
              value={query}
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="space-y-3">
              {filtered.map((row) => {
                const Icon = iconMap[row.icon as IconKey] ?? Shield
                return (
                  <div
                    key={row.id}
                  ...
                  >
                    <div className="flex flex-1 items-start gap-4">
                      <div className={`inline-flex h-12 w-12 items-center justify-center rounded-xl ${row.bg_color}`}>
