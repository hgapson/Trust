...
import { Briefcase, FileText, Globe, GraduationCap, Home, RefreshCw, Trash2 } from "lucide-react"

import { Button } from "../../../ui/button"
import { Card, CardContent, CardHeader, CardTitle } from "../../../ui/card"
import { Badge } from "../../../ui/badge"
import { AdminAudiencesApi } from "../API/audiences"
import type { Audience } from "../../home/types"

type IconKey = "Globe" | "Home" | "Briefcase" | "GraduationCap"
...
const ICON_OPTIONS: Array<{ key: IconKey; label: string }> = [
  { key: "Globe", label: "Globe — Recent Migrants" },
  { key: "Home", label: "Home — Former Refugees" },
  { key: "Briefcase", label: "Briefcase — Career Changers" },
  { key: "GraduationCap", label: "GraduationCap — Skilled Professionals" },
]
...
export default function AdminAudiencesPage() {
  ...
  const load = async () => {
    try {
      setLoading(true)
      setError(null)
      const data = await AdminAudiencesApi.list()
  ...
  const upsert = async () => {
    ...
    try {
      ...
      if (editingId) {
        const updated = await AdminAudiencesApi.update(editingId, payload)
        setRows((prev) => prev.map((row) => (row.id === updated.id ? updated : row)))
      } else {
        const created = await AdminAudiencesApi.create(payload)
  ...
  const remove = async (id: number) => {
    ...
    try {
      await AdminAudiencesApi.remove(id)
    ...
    <section className="space-y-8">
      <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900 via-slate-900/80 to-slate-950 p-6 shadow-2xl">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-semibold">Who We Serve</h1>
            <p className="mt-2 max-w-2xl text-sm text-slate-300">
              Manage the audience cards displayed on the home page.
            </p>
          </div>
      ...
      <Card className="shadow-xl">
        <CardHeader>
          <CardTitle className="text-2xl">
            {editingId ? "Edit Audience" : "Add an Audience"}
          </CardTitle>
          <p className="mt-1 text-sm text-slate-500">
            Use the icon key and styles to match the front-end cards.
          </p>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button onClick={upsert} disabled={saving}>
              {saving ? "Saving..." : editingId ? "Update Audience" : "Add Audience"}
            </Button>
            {editingId ? (
      ...
      <Card className="shadow-xl">
        <CardHeader>
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <CardTitle className="text-2xl">Current Audiences</CardTitle>
            <input
              value={query}
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="space-y-3">
              {filtered.map((row) => {
                const Icon = iconMap[row.icon as IconKey] ?? Globe
                return (
                  <div
                    key={row.id}
